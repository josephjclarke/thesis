...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
where $A,B$ have eigenvalues with zero and negative real parts respectively and $p,q$ are at least quadratic. Then the centre manifold can be written as a graph $W_c = (\bm{w},h(\bm{w}))$
...
this is shown in \cref{fig:stommel_amoc}.\todo{redo this figure!} Near the bifurcation point, the flux behaves like $\Psi \sim \sqrt{\eta_2^*-\eta_2}$,
...
$\bm{\eta}(t) = \dv*{\bm{W}}{t}$ is the derivative of the Weiner process and is the source of the stochasticity.
...
\end{equation}
where $\delta_{ij}$ is the Kronecker delta and $\delta(t-t')$ is the Dirac delta function. This notation is somewhat misleading as it suggests that all the functions involves are
differentiable, however realisations of the Weiner process are almost surely not differentiable \parencite{Mckean2014}. More rigorously, \cref{eq:general_sde} should be written as
\begin{equation}
...
\end{equation}
where $x^*$ is the equilibrium. The variance and autocorrelation are estimated empiracally from the detrended time series of $x$. In addition, the variance and

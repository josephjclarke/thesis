...
represents the saturation value of NPP at high \ce{CO2} levels. As the pre-industrial NPP, $\Pi_0$, is more accurately known than the saturation value,
...
In order to determine what $S$ should be, the soil carbon balance will be considered at every point on the Earth's surface. There will be a spatially varying amount of warming,
...
\begin{equation}
  ...
  \dv{\langle C_s\rangle}{t} \approx \left\langle \Delta \Pi - r_0 \Delta C_s \right\rangle-\alpha \langle \Pi_0 \Delta T_a \rangle.
\end{equation}
Introducing an effective temperature change $\Delta T_{\mathrm{eff}}$, defined so that
\begin{equation}
  \label{eq:motivation_of_effective_temperature}
  - \alpha \left \langle \Pi_0 \Delta T_a \right\rangle = - \alpha \left \langle \Pi_0 \right\rangle \Delta T_{\mathrm{eff}}
\end{equation}
which implies
\begin{equation}
  \label{eq:definition_of_effective_temperature}
  \Delta T_{\mathrm{eff}} = \frac{\left \langle \Pi_0 \Delta T_a \right\rangle}{\left \langle \Pi_0 \right\rangle}.
\end{equation}
After a doubling of \ce{CO2}, $\Delta T_{\mathrm{eff}} = S$ and $\langle \Delta T_a \rangle = \mathrm{ECS}$ which means that
\begin{equation}
  \label{eq:S_vs_ECS}
  \frac{S}{\mathrm{ECS}} = \frac{\left \langle \Pi_0 \Delta T_a \right\rangle}{\left \langle \Pi_0 \right\rangle \left \langle \Delta T_a \right \rangle}.
\end{equation}
...
Assuming that over land the correlation between NPP and $\Delta T_a$ is weak then
\begin{equation}
  \label{eq:S_vs_ECS_land_ocean}
  \frac{S}{\mathrm{ECS}}
  = \frac{\left\langle \Pi_0\right\rangle_{\mathrm{land}} \left\langle \Delta T_a \right\rangle_{\mathrm{land}}}{\left \langle \Pi_0 \right\rangle_{\mathrm{land}} \left \langle \Delta T_a \right \rangle_{\mathrm{global}}}
  = \frac{\left\langle \Delta T\right\rangle_{\mathrm{land}}}{\left\langle \Delta T \right\rangle_{\mathrm{global}}}
\end{equation}
...
\Cref{eq:S_vs_ECS} can be used with abrupt-4xCO2 CMIP runs \parencite{Eyring2016} to estimate $S$. Taking $\Pi_0$ to be the initial NPP in these simulations gives an estimate of $S$ of around $1.5$,

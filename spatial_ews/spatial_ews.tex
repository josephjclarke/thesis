...
to manifest themselves when viewed spatially.

To begin, consider the limit of no coupling in space, which means $D = 0$. Furthermore, assume $\epsilon$ is small enough so that $\mu$ can be taken to be a parameter of the system.
Then linearising the system about an equilibrium, $y^*$ gives
\begin{equation}
  \label{eq:linearised_spatial}
  \dv{z}{t} = -\lambda z + \sigma \zeta
\end{equation}
where $z = y - y^*$ and $\lambda = (y^*)^2 - 1$. Calculating the statistics of this system over space are equivalent to viewing each point in space as a seperate realisation of the system.
...
The Fokker-Planck equation for assossiated with \cref{eq:linearised_spatial} is
\begin{equation}
...
\end{equation}
so that as $\lambda \rightarrow 0$ at the bifurcation $\sigma_z^2$ diverges. It follows from the Fluctuation-Dissipation Theorem \parencite{Marconi2008} that
the autocorrelation at lag-$t$ is $\alpha(t) = e^{-\lambda t}$ and so $\alpha \rightarrow 1$ at the bifurcation. This is the same behaviour as in the non-spatial case.
...
\end{equation}
As $\lambda \rightarrow 0$, then $\tau_k$ increases, so critcal slowing down is still to be expected. Suppose however that only large $k$ modes were resolved, which corresponds to
...
\begin{figure}
  ...
  \caption[The quality of early warning signals in the $\epsilon$ and $D$ plane]{Red indicates regions of parameter space where the Early warning signals are reliable, blue
    where they are not. The dashed line is the line $\sqrt{D} + 40\epsilon\sqrt{D} = L$.}
...
It was shown in \cref{sec:spatial_csd} that by recognising the connection between \cref{eq:spatial_system_variational} and Landau theory for phase transitions, the two point
correlation function could be computed at the correlation length $\xi = \sqrt{D}/\sqrt{\lambda}$ computed. In order to detect any spatial variabiity the system must be sampled over
...
warning signals in space we need $D \ll L^2$ where $L$ is the size of the domain.
...
we have $\xi \approx \sqrt{D} + \frac{1}{2}\sqrt{D}\lambda^{-3/2} \left|\dot{\lambda}\right|t$. Assuming that $\dot{\lambda} = \mathcal{O}(\epsilon)$ when $t=\mathcal{O}(1)$ then it might be expected
...
In order to view the dynamics as oscillations about equilibrium, it must be the case that the forcing timescale is, $1/\epsilon$, is small compared to the timescale

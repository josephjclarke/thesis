...
Earlier in \cref{chapter:continuous_compost_bomb,chapter:global_bomb} we looked at specific examples of tipping points.
It would be useful to have motitoring systems to determine how close we are to triggering these tipping points. One way to do this is
with Early Warning Signals. These signals assume the system is forced slowly compared to its timescale. However in the case of soil carbon and
antropogenic global warming the forcing is fast. In this chapter we investigate Early Warning Signals appropropriate to this case.
...
changes quickly relative to the timescale of the system\parencite{VanderBolt2021}\todo{I should read this paper properly}.
...
problem and can then apply all the usual tools to detect approaching tipping points\parencite{scheffer2009}.

This limit of slow forcing can also be viewed as the limit of a system with a very rapid timescale. Hence the tipping point associated to the system
...
using the temporal variance and autocorrelation\parencite{VanderBolt2021}. Unfortunately these sorts of systems are widespread in climate change science.
Recall that the rate of global warming is fast, on the order of \SI{1}{\kelvin} per century\parencite{Osborn2021}, a rate unprecedented in at least
the last 2000 years\parencite{AR6}. Many important tipping elements have timescales of centuries or longer\parencite{Lenton2008}.
...
can give a better early warning than the temporal variance. Another study\parencite{Kefi2014} found rising memory, variability and changes to patchiness were all
...
the earth system have all involved temporal rather than spatial indicators\parencite{Boulton2022,Boers2021,Boers2021a}.\todo{I should read these studies\ldots}
...
variable $y$. In one dimension there is only one generic type of bifurcation\parencite{Thompson1994}, the saddle node. Furthermore
near a saddle node bifurcation, all systems show qualitatively similar behaviour\parencite{guckenheimer2013} so the dynamics of $y$ should
...
For example, it has been used in energy balance models for the global temperature\parencite{Ghil1976}, ecosystem pattern formation\parencite{Gowda2014}
and in continuum models of the compost bomb\parencite{Clarke2021}.

We will introduce noise into the system and assume the control parameter changes linearly with time. These considerations combine to motivate studying
\begin{subequations}
\label{eq:spatial_system}
  ...
  \end{align}
\end{subequations}
where $\zeta$ is a white noise with mean $\E \left[\zeta\left(x,t\right)\right] = 0$ and covariance $\E \left[\zeta\left(x,t\right)\zeta(x',t')\right] = \delta(x-x')\delta(t-t')$.
...
\end{equation}
which is a Time Dependent Ginzburg Landau equation\parencite{goldenfeld1992}.

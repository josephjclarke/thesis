...
To begin, consider the limit of no coupling in space, so $D = 0$. Furthermore, assume $\epsilon$ is small enough so that $\mu$ can be taken to be a parameter of the system. Then linearising the
system about an equilibrium, $y^*$ gives
\begin{equation}
...
In this case, the Fokker-Planck equation,
\cref{eq:fokker_planck}, can be used to calculate the statistics. The Fokker-Planck equation for assossiated with \cref{eq:linearised_spatial} is
\begin{equation}
...
the autocorrelation at lag-$t$ is $\alpha(t) = e^{-\lambda t}$ and so $\alpha \rightarrow 1$ at the bifurcation. This is evidently the same behaviour as in the non-spatial case.
...
\end{equation}
\Cref{eq:linearised_spatial_D} can be decomposed via a fourier series
\begin{equation}
  \label{eq:fourier}
  z(x) = \frac{1}{L} \sum_k z_k e^{ikx}
\end{equation}
to give
\begin{equation}
  \label{eq:linearised_spatial_D_modes}
  \dv{z_k}{t} = -\lambda z - Dk^2 z + \sigma \zeta_k
\end{equation}
Each mode with wavenumber $k$ relaxes to equilibrium on a timescale
\begin{equation}
...
To investigate the effects of critical slowing down on variance and autocorrelation, the probability distribution, $p[z,t]$ which is now a function of $z$,
of \cref{eq:linearised_spatial_D} must be calculated. This system has its own
form of the Fokker-Planck equation which describes the evolution of $p[z,t]$,
\begin{equation}
...
\begin{equation}
  ...
  G(x_1 - x_2) = \E z(x_i)z(x_j).
\end{equation}
This function can be obtained by differentiation. After introducing the extra term $-B(x)z(x)$ into the integral in \cref{eq:linearised_energy}, the two point correlation function

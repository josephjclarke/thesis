...
seen. Therefore over a timescale $\mathcal{O}(\epsilon)$, $\exp\left(-t/\tau_k\right) \approx 1$ which requires $\epsilon \ll \tau_k$. Requiring that this holds for wavenumbers corresponding to the scale of the domain
(which is when $\tau_k$ is largest) gives the requirement that
\begin{equation}
  \label{eq:scaling_eps}
  \epsilon \ll \frac{1}{1 + \frac{16\pi^2D}{L}}
\end{equation}
where again it is assumed that $\lambda = \mathcal{O}(1)$ and $k$ corresponds to modes with wavelength equal to half the domain. \todo{check this}
...
Also plotted in \cref{fig:parameter_plane} are the  lines $D = L^2/4$ and $\epsilon = \tau_{4\pi/L}$. It can be seen that they bound
...
when \cref{eq:scaling_corr,eq:scaling_eps} are violated. Instead they will become progessively less reliable, and so these scaling arguments should overestimate

...
\end{figure}
 The left column shows the small $\epsilon$ case. This is the case
    ...
    We sample 100 points\todo{did I?} in each direction, geometrically spaced, giving $10^4$ total points.
  ...
  Analysising the system spatially is aided by recognising the connection between \cref{eq:spatial_system_y_variational} and Landau theory for the Ising
  ...
  large compared to $\xi$. Hence we only expect to get good early warnings when $D \ll L^2$.
    ...
    \begin{figure}
      ...
      \caption[A schematic showing the complemenarity of spatial and temporal early warning signals]{Figure 4: A schematic illustrating the complementarity of early warning signals in space and time.
        The lines are drawn according to the scalings given in \cref{sec:scaling_arguments}}
        \label{fig:idealised_plot}
    \end{figure}

    However, there are some issues when looking at early warning signals over space. We have only 
    considered one functional form of coupling, but others are possible. Secondly, we have made
    assumptions that space is homogenous, but if $D$ is itself a function of $x$ then this could
    introduce problems.
\section{Conclusion}

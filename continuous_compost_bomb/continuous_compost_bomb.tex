...
Here I focus on the compost bomb instability in response to the seasonal cycle. In this case the timescale of the forcing (1 year) is much faster than the response timescale of the soil carbon
(decades), and the soil carbon can be treated as a prescribed time-invariant quantity (i.e.\ I consider the `compost bomb limit' of LC10). 
...
Despite these caveats, the LC10 model captures the essence of the system. T aim is to add realism to the LC10 model by considering the vertical structure and heat conductivity of the soil.
I model a one dimensional soil column in which heat can diffuse and soil carbon decreases exponentially with depth. I investigate whether an instability still exists in this model.
...
rate-induced tipping by the sinusoidal variations in air temperature caused by the diurnal and seasonal cycles. I investigate those possibilities here, to see if there exist
...
$T_s$ is `excited' to a very large value, well in excess of \SI{100}{\degreeCelsius}. I further note that this model implies a value for the equilibrium soil carbon
\begin{equation}
...
I investigate the effects of the representation of spatial variability in the vertical $z$ (\si{\meter}) direction, which extends from the
surface at $z = 0$ down to $z = -\infty$. I assume soil carbon falls exponentially with depth, over a characteristic distance of $H$.
I model soil temperature as a reaction-diffusion system, in which heat is generated
...
it has heat capacity $\mu_V$ (\si{\joule\per\cubic\meter\per\kelvin}) and contains a total amount $C_s$ of soil carbon. I can therefore write the heat diffusion
...
I set $C_s$ to the equilibrium value using \cref{eq:equilibrium_soil_carbon} where the air temperature is taken to be $T_{a0}$. This approximation
is justified provided I work on timescales short relative to the turnover time of soil carbon, which is on the order of many decades\cite{Varney2020}. 
...
I considered the instability to occur when scipy's solver could not find a solution.
...
I plot the temperature profile of soil at different times, initialised to be in equilibrium, undergoing the compost bomb thermal runaway after five months.
I set $\delta T_a$ to a  a sinusoid of frequency one year.
\Cref{fig:vertical_profiles} shows that the instability remains in the continuous case, and it is not prevented by diffusion.
...
In this subsection I compare the LC10 and continuum models numerically. The level of soil carbon is chosen such that the soil temperature is initially in an equilibrium state with $\delta T_{a} = 0$.
I then integrate forward in time with $\delta T_a$ constant and greater than zero. I find for sufficiently large values of $\delta T_a$ the system has an instability.
I refer to the smallest value of $\delta T_a$ for which this is true as the `critical warming level', as if air temperatures were increased by this amount quickly with respect to the soil carbon turn over time,
an instability would be triggered.

I plot this critical warming, as a function of $\kappa$ in \cref{fig:comparison_with_lc10}. I also overlay the warming required to cause a compost bomb in the LC10
model. Also shown is the amplitude of the seasonal cycle needed to cause a compost bomb.
I see that as $\kappa\rightarrow\infty$ the vertical structure matters less and the continuum result asymptotes to the LC10 case. In addition, I see that the soil is more stable
...
\begin{figure}
  ...
  \caption[Critcal warming level in the continuum compost bomb]{The warming required to make the continuum model unstable as a function of $\kappa$, compared to the LC10 model. I have also plotted on the seasonal cycle amplitude required for this instability.}
  \label{fig:comparison_with_lc10}
\end{figure}

To investigate this further, I derive  the LC10 model from the continuum case. I do this by vertically averaging \cref{eq:one_d_model}.
I denote vertically averaged quantities as
$\left\langle q \right\rangle = \frac{1}{H}\int_{-\infty}^0 q \mathrm{d}z$. Neglecting the vertical dependence of soil carbon, and
taking a vertical average, applying boundary conditions, and finally replacing $\mu_A$ with $\mu_V H$ I get
\begin{equation}
...
Now that I have numerical evidence for a compost bomb in the continuum case, I investigate this analytically. In the approximation where soil carbon is constant,
I lose the rate dependent tipping feature of the model, and instead it reverts to a classical bifurcation-induced tipping problem.
I consider the case where $\delta T_a$ is a constant, given by the temperature increase relative to the long term background state, $T_{a0}$, which I refer to as an atmospheric warming.
In this case $\delta T_a$ is the bifurcation control parameter.
I begin by making the following nondimensionalisations:
\begin{subequations}
...
I now show that the model defined by \cref{eq:nondimensional_model_nonuniform} only has an equilibrium state for low levels of atmospheric warming, $\delta\theta_a$.
I make a change of variables which lets us find the first integral of the steady-state of 
...
I assume $\delta\theta_a$ is constant, set $\partial_{\tau}\theta = 0$, and then let $\psi = \theta + x/D$. Then \cref{subeq:nondimensional_model_nonuniform_reaction_diffusion} becomes
\begin{equation}
...
\end{equation}
Denoting the left-hand side of this equation as $F(c_2)$, it can be shown that $F$ has a minimum. Hence for sufficiently large $\delta\theta_a$, the right-hand side will always
...
I find the $c_2$ that satisfies $F'(c_2) = 0$ and substitute this back into \cref{eq:nondimensional_model_nonuniform_upper_boundary_condition}:
\begin{equation}
...
\end{equation}
where I have used the fact that the logarithm of a negative number is not real.
Note that for sufficiently large values of $\mathcal{W}$, the right-hand side of this equation is negative, which I can interpret as the soil being inherently unstable without any warming.
...
\cref{eq:nondimensional_model_nonuniform_critical_warming}. I plot the critical maximum warming obtained in \cref{fig:critical_theta_a} for a range of $\widetilde{\Pi}$ and $D$ values.
...
\begin{figure}
  ...
  \caption[Critical warming to trigger a compost bomb]{\textbf{Panel a}: the relationship between nondimensional soil carbon $e$-folding depth $D$, nondimensional NPP $\widetilde{\Pi}$ and the critical warming required to cause a compost
    bomb $\delta\theta_a$. \textbf{Panel b}: I also plot a dimensional version using the standard parameters in \cref{tab:standard_values}}
...
carbon) are the diurnal and seasonal cycles. I let $\delta T_a$ vary sinusoidally,
\begin{equation}
...
I plot these in \cref{fig:forcing_frequency_vs_amplitude}.
\begin{figure}
  \centering
...
have an effect deeper in the soil. Hence I do not expect compost bombs except at very high and unrealistic amplitudes.

In particular from \cref{fig:forcing_frequency_vs_amplitude}, I see that
...
I investigate the possibility of a seasonal cycle triggered compost bomb in \cref{fig:critical_amplitude_to_be_triggered_by_the_seasonal_cycle}, scanning across our two nondimensional parameters.
...
\begin{figure}
  ...
  \caption[Critical seasonal cycle for a compost bomb]{\textbf{Panel a}: the relationship between nondimensional respiration, soil depth and the seasonal cycle amplitude required to cause a compost bombs.
    For typical values of $Q_{10}$, the seasonal cycle could trigger a compost bomb. \textbf{Panel b}: I also plot the dimensional version, here I modify the standard value
...
The compost bomb instability was originally identified in a single box model. Here I present a substantial advance in realism,
...
I attribute this difference, at least in part, to strong nonlinearity in respiration heat release, due to its exponential (`$Q_{10}$') temperature response. 
For all finite values of thermal conductivity, I find these temperature thresholds are lower than those in the LC10 model. Nondimensionalisation supports this balance of two dominant factors,
...
I have created a deliberately simplified model of the compost bomb. This has the advantage of being analytically tractable, yet ignores certain processes. Most obviously, I assume soil carbon is in
equilibrium. This is not an unreasonable assumption however as I consider short enough timescales where this is approximately true. More importantly I neglect the role of soil moisture. Although it is
beyond the scope of this paper to analyse this thoroughly I offer some justification for why our model captures the essential features of the system. The addition of soil
...
Secondly soil moisture affects heat transport through phase changes, which is a second order effect and so I are justified in ignoring it, and also through setting the conductivity of the soil.

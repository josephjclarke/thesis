...
Despite these caveats, the LC10 model captures the essence of the system. We aim to add realism to the LC10 model by considering the vertical structure and heat conductivity of the soil.
...
% This mathematical investigation aims to enhance our confidence in the existence of the compost bomb effect and to determine any dependences on near-surface
% air temperature forcing, with the overall aim of helping researchers identify plausible compost bomb event candidates in measurement records.
...
of respiration, $\mu_A$ (\si{\joule\per\square\meter\per\degreeCelsius}) the areal soil heat capacity and $\lambda$ (\si{\watt\per\square\meter\per\degreeCelsius})
...
by heterotrophic respiration and diffused vertically. The conductivity of the soil is given by $\kappa$ (\si{\watt\per\meter\per\degreeCelsius})\cite{Best2005},
it has heat capacity $\mu_V$ (\si{\joule\per\cubic\meter\per\degreeCelsius}) and contains a total amount $C_s$ of soil carbon. We can therefore write the heat diffusion
...
We set $C_s$ to the equilibrium value using \cref{eq:equilibrium_soil_carbon} where the air temperature is taken to be $T_{a0}$. This approximation
is justified provided we work on timescales short relative to the turnover time of soil carbon, which is on the order of many decades\cite{Varney2020}. 

Throughout this study we undertake the mathematical investigation using nondimensional values. However, to aid understanding we plot certain figures
using dimensional units, with standard values given in \cref{tab:standard_values}. It should be noted however that these parameters are choices we have made,
...
\begin{tabular}{@{}llll@{}}
  %\toprule
  Parameter & Name & Equation & Value                                          \\ \midrule
  $\Pi$     & Net Primary Productivity & \ref{subeq:lc10_soil_carbon} &\SI{0.5}{\kilo\gram\carbon\per\year}           \\
  $Q_{10}$   &  $Q_{10}$ value for response of specific respiration to temperature &\ref{eq:one_d_model} & 2.5  \\
  $H$       & Characteristic Soil Depth &\ref{eq:one_d_model} &\SI{0.4}{\meter}                               \\ 
  $\kappa$  & Soil Thermal Conductivity &\ref{eq:one_d_model} &\SI{0.16}{\watt\per\meter\per\degreeCelsius}          \\
  $A$       & Specific Heat of Respiration &\ref{eq:one_d_model} &\SI{3.9e9}{\joule(\kilo\gram\carbon)^{-1}}        \\
  $\mu_V$   & Volumetric Heat Capacity&\ref{eq:one_d_model} &\SI{1.0}{\mega\joule\per\cubic\meter\per\degreeCelsius} \\
  $\lambda$ & Heat Transfer Coefficient & \ref{subeq:dimensional_bcs_nonuniform_upper} &\SI{10}{\watt\per\square\meter\per\degreeCelsius} \\
  $T_{a0}$  & Average Air temperature   & \ref{subeq:dimensional_bcs_nonuniform_upper} &\SI{0.0}{\degreeCelsius}
\end{tabular}
\caption{\label{tab:standard_values}Parameter values used to produce the figures in this study.}
\end{table}
...
\begin{figure}
  \centering
  %\includegraphics[keepaspectratio,scale=0.5]{lc10_vs_continuum_dimensional}
  \includegraphics[keepaspectratio,scale=0.5]{dimensional_continuum_vs_lc10}
  % \caption{The warming required to make the continuum model unstable as a function of $\kappa$, compared to the LC10 model. We have chosen $\Pi$ = \SI{0.2}{\kilo\gramcarbon\per\square\meter\per\year} and $Q_{10} = 2.0$.}
...
To investigate this further, we derive  the LC10 model from the continuum case. We do this by vertically averaging \cref{eq:one_d_model}.
...
We now show that the model defined by \cref{eq:nondimensional_model_nonuniform} only has an equilibrium state for low levels of atmospheric warming, $\delta\theta_a$.
...
\begin{figure}
  \centering
  % \includegraphics[scale=0.5,keepaspectratio]{critical_cycle_as_function_of_frequency.pdf}
...
\begin{figure}
  \centering
  %\includegraphics[scale=0.5,keepaspectratio]{critical_theta_a_for_seasonal_cycle.pdf}
  ...
  \caption{\textbf{Panel a}: the relationship between nondimensional respiration, soil depth and the seasonal cycle amplitude required to cause a compost bombs.
    ...
    of $\kappa$ to \SI{0.5}{\watt\per\meter\per\degreeCelsius} so make the $H$ values more realistic.}
...
Secondly soil moisture affects heat transport through phase changes, which is a second order effect and so we are justified in ignoring it, and also through setting the conductivity of the soil.

...
This runaway potential was first investigated in \cite{Luke2011}. The Luke and Cox model (hereafter referred to as the LC$10$ model)
...
(decades), and the soil carbon can be treated as a prescribed time-invariant quantity (this is the `compost bomb limit' of LC$10$). 

Some limitations of the LC$10$ model are that it neglects important thermal processes and soil structure, and in particular vertical variation.
...
that diffusive damping may make the compost bomb harder to trigger. Additionally, the LC$10$ model assumes a single pool of carbon, rather than a spatially extended distribution, which might
increase the possibility for a compost bomb.

Despite these caveats, the LC$10$ model captures the essence of the system. The aim is to add realism to the LC$10$ model by considering the vertical structure and heat conductivity of the soil.
...
\section{LC$10$ Single Box Conceptual Model}
\label{sec:lc10}
...
the soil-to-atmosphere heat transfer coefficient leads to the LC$10$ model:
\begin{subequations}
...
\begin{table}
  ...
  Specific Heat of Respiration &$A$       &\ref{eq:one_d_model} &\SI{3.9e7}{\joule\kilo\gram\per\carbon}        \\
...
This is a first piece of evidence that the results of the LC$10$ model remain when soil depth is taken into account. \Cref{fig:vertical_profiles}
...
\end{figure}
\subsection{Consistency of the continuum model with LC$10$}
\label{sec:consistency_with_LC$10$}
In this subsection the LC$10$ model and continuum model are compared numerically. The level of soil carbon is chosen such that the soil temperature is
...
Also overlaid is the warming required to cause a compost bomb in the LC$10$
model. Also shown is the amplitude of the seasonal cycle needed to cause a compost bomb.
It can be observed that as $\kappa\rightarrow\infty$ the vertical structure matters less and the continuum result asymptotes to the LC$10$ case. In addition, the soil is more stable
...
Surprisingly for smaller values of $\kappa$ the critical warming is actually lower in the continuum case than the LC$10$ case,
despite the damping effects of diffusion.
\begin{figure}
  \centering
  \includegraphics[keepaspectratio,width=\textwidth]{dimensional_continuum_vs_lc10}
  \caption[Critical warming level in the continuum compost bomb]{The warming required to make the continuum model unstable as a function of $\kappa$, compared to the LC$10$ model.
    Also plotted on is the seasonal cycle amplitude required for this instability.}
  \label{fig:comparison_with_lc10}
\end{figure}

To investigate this further, the LC$10$ model will be derived from the continuum case. This is done by vertically averaging \cref{eq:one_d_model}.
...
\end{equation}
This is equivalent to the LC$10$ model if $T_s(0,t) \approx \left\langle T_s \right\rangle$ and $\left\langle e^{\alpha T_s}\right\rangle \approx e^{\alpha \left\langle T_s \right\rangle}$.
...
Note however that because $\left\langle e^{\alpha T_s} \right\rangle \ge e^{\left\langle \alpha T_s \right\rangle}$ the LC$10$ model underpredicts the amount of respiration
in soils when $\kappa$ is finite, and so the LC$10$ model will need a higher amount of warming to trigger a compost bomb.
...
Consider the case where $\delta T_a$ is a constant, given by the temperature increase relative to the long term background state, $T_{a0}$, which is referred to as an atmospheric warming.
...
these values are $D = 25$ and $\mathcal{W} = 0.0001$.
...
\end{equation}
where $c_2$ is another integration constant. The left hand side is then reduced to a standard integral \parencite{riley2006}
with the substitution $u = \left(1 - \frac{2\mathcal{W}}{c_1} e^{\psi}\right)^{1/2}$, giving
\begin{equation}
  \int \frac{\mathrm{d}\psi}{\sqrt{1 - 2\mathcal{W}c_1^{-1} e^{\psi}}} = \int \frac{2}{u^2 - 1} \,\mathrm{d}u = -2\artanh u. 
\end{equation}
Inverting this then leads to
\begin{equation}
  \sqrt{1 - 2\mathcal{W}c_1^{-1} e^{\psi}} = \tanh\left(c_2 \pm \sqrt{c_1} x\right),
\end{equation}
where $c_1$ and $c_2$ have been redefined to absorb the factor of -2. As a result, $\psi$ can be expressed as
\begin{equation}
...
for $|c_2| \rightarrow \infty$ behaves like $F(c_2) \sim |c_2|$ so it follows %from Rolle's Theorem\parencite{Bourbaki2004}
...
Using the parameters in \cref{tab:standard_values} and setting $\delta \theta_a = 0$,
\cref{eq:nondimensional_model_nonuniform_upper_boundary_condition} was numerically solved for $c_2$ and thus the equilibrium distribution of $\theta$ was found.
This was then converted back to dimensional units and plotted in \cref{fig:equilibrium_soil_temperature_profile}. Much like observations \parencite{Ping1987},
the temperature was found to be nearly constant for depths below about \SI{1}{\meter}. Near-surface soil is in good thermal contact with the atmosphere, the temperature of
which varies over short timescales, and therefore it is unlikely that observed near-surface soil temperatures will have reached their equilibrium value.
Nevertheless, the profile here is compatible with soil temperature observations.

\begin{figure}
  \centering
  \includegraphics[keepaspectratio,width=\textwidth]{equilibrium_profile}
  \caption[Equilibrium Soil Temperature Profile]{The equilibrium soil temperature profile, using the parameters in \cref{tab:standard_values}.
    This is given as an anomaly relative to $T_{a0}$. The soil was assumed to be \SI{4}{\meter} deep and that $\delta T_a$ was zero.}
  \label{fig:equilibrium_soil_temperature_profile}
\end{figure}
...
\begin{figure}
  \centering
  \includegraphics[keepaspectratio,width=\textwidth]{static_dim_and_nondim}
  \caption[Critical warming to trigger a compost bomb]{\textbf{Panel a}: the relationship between nondimensional soil carbon $e$-folding depth $D$,
    ...
    The black crosses denote the point corresponding to using the NPP and $H$ values from \cref{tab:standard_values}.}
...
thermal runaway still occurs. For all finite values of thermal conductivity, I find these temperature thresholds are lower than those in the LC$10$ model, which I attribute
...
warming levels required to initiate the instability are much smaller than in the LC$10$ case. For the set of parameters investigated a rapid warming of the order of \SI{10}{\degreeCelsius} is

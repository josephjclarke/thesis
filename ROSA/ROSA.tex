...
The method looks at the {\it Ratio of Spectra (ROSA)} of a system state variable relative to a forcing variable.   
...
increases the chance of a false positive as that quantity {\pr may change for other reasons such as an increase in the noise variability.} It should 
...
{\pr if a transition is noise-induced (rather than bifurcation-induced) then}
critical
speeding up {\pr is possible, where}
%, leading to 
a decrease in the variance and AC can
be signs of an approaching transition\cite{Titus2020}.

The technique of {\pr observing an increase in} the variance and
...
\caption{Panel A shows two time series of system {\pr (\ref{eq:dynamical_system})--(\ref{eq:discretized_white_or_red_noise}), with $\mu(t)=t$} passing through a saddle node bifurcation at time {\pr $t=2/3$}. 
  In the case of the blue curve the system
  is subject to white noise forcing {\pr ($r=0$)}, the red curve is subject to 
  red noise forcing {\pr ($r=0.99$)}. Panels B and C show the
  classic EWS for the case of white and
  red noise, calculated after a
  quadratic detrend in a window of width $0.2$ (in normalised time units {\pr or 500 data points}).
...
\end{equation}
where we $\epsilon$ {\pr defines the timescale of the system, $\mu$ is a control parameter and $\eta$ provides the noise.} We note that there is a saddle node bifurcation
...
EWS suggest that {\pr the system} should become more
autocorrelated. However, if {\pr the system is subjected to} autocorrelated noise,
...
To illustrate this, we set $\mu(t) = t$, so that system {\pr (\ref{eq:dynamical_system})} has
...
the white noise case, giving {\pr a clear} Early Warning Signal. 
{\pr Whereas, for the red noise case there is no trend in the AC. The 
...
(\ref{eq:dynamical_system}) with $\mu = -1$, hence there {\pr is} no 
tipping point. The stochastic forcing $\eta${\pr , described by (\ref{eq:white_red_forcing}),} has its $r$ value
linearly increasing
...
{\pr Therefore}, using this method requires an understanding
of the system. Furthermore the system must be {\pr of sufficient temporal resolution such} that 
...
{\pr We} calculate the power spectra of $x$ and $\eta$
...
\ref{fig:failure_of_ews} and \ref{fig:changing_forcing} {\pr respectively}.
...
subject to white or red noise. Panel D {\pr provides} 
the value of $\lambda$ estimated from ROSA {\pr and shows a clear}
...
false positive. In Panel C we plot $\lambda$ obtained from ROSA, {\pr which stays constant and} close to the true value (plotted in the dashed line). ROSA therefore avoids a false positive in this case.




\section{Comparison to alternative methods}
{\pr Recently}, Boers\cite{Boers2021a} introduced a new technique,
...
regressing $\dot{x}$ against $x$, {\pr to give} an estimate for $\lambda$. 
{\pr This} requires a model for the noise, for example that it is
generated by an Ornstein-Uhlenbeck process. {\pr Significantly, the BB method makes a quasi-static assumption, such that the} parameters of the {\pr noise model are assumed to be fixed in each window (although can change between windows). On the other hand, ROSA does not have such a restriction because the noise is analysed directly.}
...
$\tau$ near the tipping point. However, ROSA becomes positive substantially earlier than the BB method and there is less variance in the $\tau$ values for the ROSA method, as required for a more reliable early warning indicator. This is indicative of {\pr an earlier} and stronger warning than the BB method. 

Nevertheless, the BB method has {\pr its advantages}.
It is flexible and straightforward to modify to different noise
models {\pr and in many cases gives a} good Early Warning Signal 
{\pr ahead of} the tipping point. Its principle advantage
...
where {\pr data is scarce such as} in paleoclimate studies, BB is more suitable but 
...
examine Amazon forest dieback in state-of-the-art ESMs from the Coupled Model Intercomparison  Project - Phase 6 (CMIP6)\cite{Eyring2016} database. {\pr Recently, five (EC-Earth3-Veg, GFDL-ESM4, NorCPM1, SAM0-UNICON and TaiESM1) of seven models, which possess a Dynamic Global Vegetation Model
...
{\pr was} detrended similarly to the vegetation carbon. 
...
of detecting substantial proportions of the {\pr identified} abrupt shifts. {\pr Overall}, we see that ROSA outperforms the traditional EWS. {\pr For} individual models,
ROSA detects more abrupt shifts than the AC alone and is often better than the 
variance. Hence, for most of the CMIP6 models considered, ROSA is able to give a{\pr more robust}
...
In this paper we {\pr have} shown how EWS can be generalised to deal with more general noise characteristics. By normalising the power spectrum of the forcing, we are able to extract a 
...
requires {\pr having} data {\pr for a sufficiently} long period of time, which may 

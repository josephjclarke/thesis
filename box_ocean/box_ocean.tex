...
However the latest generation CMIP$6$ climate models have climate sensitivities up to \SI{5.6}{\kelvin} \parencite{Zelinka2020} and some climate models can give sensitivities of over
...
\begin{table}
   ...
   \begin{tabular}{llll}
     \toprule
     \multicolumn{2}{c}{CMIP$5$}               & \multicolumn{2}{c}{CMIP$6$}            \\
     ...
     \bottomrule
   \end{tabular}
\caption[Estimates of $S/\mathrm{ECS}$ for CMIP models]{Estimates of the ratio of $S$ to $\mathrm{ECS}$ for CMIP$5$ and CMIP$6$ models, calculated by the methodology described in \cref{chapter:global_bomb}.
...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
As the dynamics are highly non-trivial, the function $I$ has been used to represent the IMOGEN model. $\Delta T_o$ is the change in ocean temperature which is related to the change in land temperature by the
...
changes in atmospheric \ce{CO2}. Secondly, I have neglected any temperature dependence on NPP as well as the spatial structure of the land response.
Furthermore, IMOGEN's representation of ocean carbon is not `processed based', but is instead based on fitting an impulse-response function to a GCM.
...
\begin{figure}
  ...
  \caption[Using IMOGEN to find the behaviour of atmospheric \ce{CO2} as a function of $S$]{Atmospheric carbon calculated with the IMOGEN model
...
\begin{figure}
  \centering
  \includegraphics[keepaspectratio,scale=0.5]{imogen_summary}
...
As some CMIP$6$ models have an $S \approx\SI{9}{K}$ this imposes limits on the strength of their carbon cycle feedbacks. Furthermore, because these models are
...
of integration can be set to zero. \Cref{eq:ocean_greens_function} is similar to \cref{eq:imogen_impulse_response}, except the short term behaviour differs qualitatively
...
$C_a = C_{a0}$, $C_o = 0$ and $C_s = C_s^{\mathrm{eq}}$. It will be shown that under suitable conditions there is another fixed point.

\subsection{Stability of Pre-industrial State in the One Box Ocean Model}
...
For this section, it will be assumed that $C_2$ has a slow response to a given atmospheric \ce{CO2} forcing, when compared to $C_1$. This implies that $\epsilon \ll 1$.
...
\begin{figure}
  ...
  linearly with $\mu$ from around \SI{23}{\year} to \SI{64}{\year}. These periods are comparable, at least near the critical value of $\mu$,
...
That the second timescale obtained from observations is much shorter than required to make the bifurcation points in the two box and IMOGEN models match may imply that the record is not

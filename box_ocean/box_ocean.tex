...
I assume Net Primary Priductivity is a function of the atmospheric carbon only that saturates at high \ce{CO2}, having the form
\begin{equation}
  \label{eq:npp}
...
however here I will make use only of the ocean component. Similarly the box models previously described, IMGOEN's impulse response function to eleveated \ce{CO2} is a sum of exponentials,
in this case six.
However it is not just integrated against the change in \ce{CO2} but instead a nonlinear function of atmospheric carbon, ocean carbon and ocean temperature. The climate carbon system with IMGOEN
...
near this bifurcation point, they will be subject to critical slowing down and this will affect their variability. In \cref{fig:imogen_ca_variability,fig:imogen_ca_autocorrelation}
...
\begin{subequations}
  ...
  \begin{align}
    ...
    \dv{C_o}{t}     &= \mathrm{IMOGEN}(C_a,T_o,C_o) \\
    c\dv{T}{t}       &= -\gamma\frac{Q_{2\times}}{S} T + \frac{Q_{2\times}}{\log 2} \log \frac{C_a}{C_{a0}} + \sigma_Q \xi_
    C_a             &= \mathcal{C} - C_s - C_o \\
    T_o             &= \frac{1}{\nu}\frac{S}{\log 2}\log \frac{C_a}{C_{a0}}
    T_s             &= \gamma T
  \end{align}
  where $S = \gamma \mthrm{ECS}$, $c = \SI{10.0}{\watt\year\per\meter\squared\per\kelvin}$ is the heat capacity, $\xi$ is white noise

...
the global mean temperature assume that $N(T)$, the net radiation received by the Earth, depends on $T$. In equilibrium $N$ is zero. If there is
a small radiative forcing $\Delta Q$ then $T$ must change to ensure $N = 0$. The change in global mean temperatures is
\begin{equation}
  \label{eq:response_to_radiative_forcing}
  \Delta T = -\pdv{N}{T}^{-1} \Delta Q.
\end{equation}
The quantity
\begin{equation}

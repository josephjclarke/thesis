...
B- or Bifurcation-tipping refers to tipping which is driven by changes to the stability, or the loss alltogether, of these fixed points.  A bifurcation is a concept from the mathematical theory of dynamical systems,
...
Consider a modification to \cref{eq:generic_system} where a control parameter $\bm{\mu} \in \mathbb{R}^k$ has been introduced
\begin{equation}
  \label{eq:generic_system_control_parameter}
  \dv{\bm{x}}{t} = \bm{f}_{\bm{\mu}}(\bm{x}).
\end{equation}
The fixed points are described by $\bm{f}_{\bm{\mu}}(\bm{x}^*) = 0$. By the Implicit Function Theorem, $\bm{x}^*$ is a smooth function of $\bm{\mu}$ except at points where $J$ has a zero eigenvalue,
these points in $(\bm{x},\bm{\mu})$ space are known as bifurcation points. Other bifurcation points can occur when a stable fixed point becomes unstable (or vice versa). In both cases
...
Center manifold theorem. Normal Forms. Saddle node. Stommel Example. Put Stommel in Normal form?

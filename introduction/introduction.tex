...
However not all types of tipping will give good early warning signals. Noise and shock tipping are not driven by an external factor and so early warning signals (in the conventional sense)
...
In this case the important statisitcs are
\begin{align}
...
\item Perform statistical tests to detect if $\sigma^2_{y_i}$ and  $\alpha_{1}$ are increasing.
\item If they are increasing, then this can be taken as an early warning of an approaching B-tipping.
\end{enumerate}

Timescales? Statistical tests?

\begin{figure}
  \centering
  \includegraphics[width=\textwidth,keepaspectratio]{ews}
  \caption[An example of an early warning signal]{An example of using early warning signals to detect an approaching tipping point. In the top left is the original time series. The top right shows the time series detrended using a cubic polynomial. The bottom left shows the variance and the bottom right shows the autocorrlation.}
  \label{fig:ews}
\end{figure}

...
\Cref{eq:simple_sde} has an assossiated Fokker-Planck equation
\begin{equation}
...
a situation where the rate of change of a control parameter, rather than the magnitude of the parameter itself  controls whether a system tips or not. This type of tipping
can only occur in nonautonomous dynamical systems (a nonautonomous system depends explicitly on time).
...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
These equations can be interpretted as a system with state $x$ in a changing environment represeted by parameter $\mu$ that changes at some finite rate, $r$.
...
Consider the `frozen system' \parencite{W?}.
This is \cref{eq:rtipping_example_x} with $\mu$ constant --- this describes the dynamics in the case where the environment is unchanging. It has equilibria at
$x^*_{\pm} = \pm 2 - \mu$. The stability of these equilibria can be found in the matter descibed in \cref{sec:btipping}, which in this case amounts to evaluating
...
unstable for all $\mu$ values. These quasi-static equilibria are shown as grey lines in \cref{fig:rate_tipping}, where the stable quasi-static equilibria is shown in
solid grey and the unstable quasi-static equilibria is shown as a dashed line. As a result a naive B-tipping analysis would suggest this system cannot tip, however it can undergo R-tipping.
...
pullback attractor and repellor of the system, $x^{\mathrm{PB}}_{\pm} = \pm \sqrt{4 - r} - \mu$. These lines are plotted in black in \cref{fig:rate_tipping} where the solid line
...
If $r < 4$, then $x_-^{\mathrm{PB}}$ exists and so solutions head towards it. However if $r > 4$ then $x_-^{\mathrm{PB}}$ does not exist and so solutions diverge to infinity. As a result there
is a critical rate, $r_c = 4$ above which R-tipping occurs.

\subsubsection{Example}
The paradigmatic example of R-tipping is the Compost Bomb instability \parencite{Luke2011,Wieczorek2011,Clarke2021,OSullivan2022arxiv}, which is an thermal
...
\Cref{fig:compost_bomb_example} shows two time seris of \cref{eq:compost_bomb} with $\dot{T}_a = \SI{0.05}{\kelvin\per\year}$ and $\dot{T}_a = \SI{0.1}{\kelvin\per\year}$. For the larger
...
In this sense it is closesly related to ideas of resilience introduced by~\cite{Holling1973}.
...
\begin{figure}
  ...
  \caption{An example of shock-tipping. The system given by \cref{eq:allee_effect} recieves a perturbation of $0.99$ at time $25.0$. This is not large enough to make it cross the basin boundary
